...
func AuthRoutes(route *gin.Engine) {
	...
	// ====== ADMIN ROUTES ======
	admin := route.Group("/admin").Use(middleware.RequireAdmin())
	{
		...
		admin.GET("/users", func(c *gin.Context) {
			...
			if err != nil {
				c.JSON(500, gin.H{"success": false, "error": "Failed to count users"})
				return
			}
			...
			if err != nil {
				c.JSON(500, gin.H{"success": false, "error": "Failed to fetch users"})
				return
			}
			defer cursor.Close(ctx)
			...
			if err = cursor.All(ctx, &users); err != nil {
				c.JSON(500, gin.H{"success": false, "error": "Failed to decode users"})
				return
			}
		...
		// PUT /admin/approve/:id - Approve a user (admin approval)
		admin.PUT("/approve/:id", func(c *gin.Context) {
			ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
			defer cancel()
			...
			if err != nil {
				c.JSON(500, gin.H{"success": false, "error": "Failed to approve user"})
				return
			}
		...
		admin.PUT("/users/:id/status", func(c *gin.Context) {
			...
			if err != nil {
				c.JSON(500, gin.H{"success": false, "error": "Failed to update user status"})
				return
			}

			c.JSON(200, gin.H{
				"success": true,
				"message": fmt.Sprintf("User %s successfully", map[bool]string{true: "activated", false: "deactivated"}[req.IsActive]),
			})
		})
		...
		admin.GET("/stats", func(c *gin.Context) {
			...
			totalUsers, _ := database.UserCollection.CountDocuments(ctx, bson.M{"role": "user"})
			activeUsers, _ := database.UserCollection.CountDocuments(ctx, bson.M{"role": "user", "is_active": true})
			inactiveUsers, _ := database.UserCollection.CountDocuments(ctx, bson.M{"role": "user", "is_active": false})
			subscribedUsers, _ := database.UserCollection.CountDocuments(ctx, bson.M{"role": "user", "has_subscription": true})
