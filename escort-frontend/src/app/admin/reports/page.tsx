...
export default function SubscriptionsPage() {
  ...
  const API_URL = process.env.NEXT_PUBLIC_API_URL || https://escort-vcix.onrender.com/users;
  ...
  const fetchSubscriptions = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        // If the API returns { subscriptions: [...] }
        ...
        subscriptionsArray = Array.isArray(data) ? data : [];
      }

      console.log('Subscriptions data:', data); // Debug log
      console.log('Processed subscriptions:', subscriptionsArray); // Debug log
  ...
  const filteredSubscriptions = subscriptionsArray.filter((sub: Subscription) => {
    const matchesSearch = 
  ...
  const activeCount = subscriptionsArray.filter((s: Subscription) => s.status === 'active').length;
  const pendingCount = subscriptionsArray.filter((s: Subscription) => s.status === 'pending').length;
  const expiredCount = subscriptionsArray.filter((s: Subscription) => s.status === 'expired').length;
      ...
      <div className="p-6">
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          ...
          <div className="mt-4">
            ...
            <p className="text-sm text-gray-500">API URL: {API_URL}/admin/subscriptions</p>
            <p className="text-sm text-gray-500">Token exists: {localStorage.getItem('token') ? 'Yes' : 'No'}</p>
          </div>
          <button
        ...
        <div className="flex space-x-2">
          ...
          <button
            onClick={() => {
              ...
              console.log('Type of subscriptions:', typeof subscriptions);
        ...
        {subscriptionsArray.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="mt-2 text-gray-500">
              There are no subscriptions in the system yet, or the API returned an empty response.
            </p>
            <div className="mt-6">
              <button
                onClick={fetchSubscriptions}
