...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // FIX: Check if response is OK (200-299) - Your backend returns 200 OK
      if (response.ok) {
        // ✅ Login successful (your backend returns 200 with id, role, token)
        console.log('Login successful!');
        
        // Save token (your backend returns token)
        if (data.token) {
          localStorage.setItem('token', data.token);
          console.log('Token saved');
        }
        
        // Create user object from response (your backend returns id and role)
        const userObj = {
          id: data.id || '',
          role: data.role || 'user',
        ...
        // Check if account needs approval (if backend returns is_active or status)
        if (data.is_active === false || data.status === 'pending') {
              ...
              <div className="ml-3">
                ...
                {error.includes('pending') && (
                  <p className="mt-1 text-sm text-red-600">
                    Contact admin for approval or check back later.
                  </p>
                )}
              </div>
            </div>
